...
//    @Scheduled(cron = "0 0 * * * *")
    ...
    public void detectCancelRateIncrease() {
        ...
        log.info("[Batch] 최근 1시간 취소율 이상 탐지 시작(redis)");

        List<CancelRateAnomalyDto> anomalies = paymentService.redisFindHourlyCancelRateAnomalies();
        if(anomalies.isEmpty()){
            log.info("[PERF] 탐지할 내역이 없습니다.");
            return;
        }
        ...
        log.info("[PERF] 레디스 스케줄러 시간 : {} ms", (t2 - t1));
